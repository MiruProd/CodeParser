...
class PromptEditDialog(QDialog):
    """
    Классическое диалоговое окно для прямой ручной правки текста системной инструкции.
    """
...
class RuleCardWidget(QFrame):
    """
    Интерактивный виджет карточки правила со встроенным чекбоксом и кнопками управления.
    """
    def __init__(self, rule_data, on_edit_callback, on_delete_callback, parent=None):
        ...
        # Кнопки быстрого редактирования и удаления
        self.btn_edit = QPushButton()
        self.btn_edit.setIcon(QIcon(get_resource_path("resources/edit.svg")))
        ...
        self.btn_delete.setIcon(QIcon(get_resource_path("resources/delete.svg")))
...
class CustomRuleAddDialog(QDialog):
    """
    Модальное окно для создания нового правила внутри конструктора.
    """
...
class PromptCreateDialog(QDialog):
    """
    Интерактивный конструктор системных промптов (Скиллов) на основе списка правил.
    """
    def __init__(self, prompt_manager, parent=None):
        ...
        # Названия вкладок и ключи категорий
        self.categories = {
            "system_role": "Роль ИИ",
        ...
        # Хранилище контейнеров для обновления списков карточек
        self.scroll_layouts = {}
    ...
    def init_ui(self):
        ...
        # Создаем многовкладочный интерфейс
        self.tab_widget = QTabWidget()
        for cat_key, cat_title in self.categories.items():
            tab_widget = QWidget()
            tab_layout = QVBoxLayout(tab_widget)
            
            # Область прокрутки для карточек правил
            scroll = QScrollArea()
            ...
            # Кнопка добавления кастомного правила под текущую вкладку
            btn_add_rule = QPushButton(" Добавить свое правило")
            btn_add_rule.setIcon(QIcon(get_resource_path("resources/add.svg")))
        ...
        # Заполняем конструктор карточками
        self.render_all_cards()

    def render_all_cards(self):
        """Перерисовывает списки карточек правил на всех вкладках."""
        for cat_key, layout in self.scroll_layouts.items():
            ...
            # Отрисовываем актуальные карточки
    ...
    def create_custom_rule(self, category_key):
        """Создает новое кастомное правило и сохраняет его в БД."""
    ...
    def on_edit_rule(self, rule_data):
        """Открывает диалог для изменения текста технической инструкции ИИ."""
    ...
    def on_delete_rule(self, rule_data):
        ...
        if reply == QMessageBox.StandardButton.Yes:
            # Находим и удаляем правило из менеджера
    ...
    def compile_and_save(self):
        """Собирает промпт из активных чекбоксов и возвращает результат в главное окно."""
        title = self.title_input.text().strip()
        if not title:
            QMessageBox.warning(self, "Ошибка", "Укажите название собираемого скилла.")
            return

        # На лету собираем активные текстовые правила по категориям
        ...
        # Сохраняем состояние активности чекбоксов на диске
        self.prompt_manager.save_rules()
        self.accept()
